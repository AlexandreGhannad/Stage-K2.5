function [outProblem] = qpstoqp(inProblem)
  % Function "qpstoqp" build the qpmodel equivalent to the problem
  ...
  %               represent a quadratic optimization problem.
  %
  % Outputs args :
  %   outProblem : a qpmodel object
  ...
  Ql = inProblem.Q;
  Qu = Ql';
  Qu(logical(speye(n))) = 0;
  Q = Ql + Qu;

  outProblem = model.nlpmodel(inProblem.name, x0, cL, cU, bL, bU, A, c, Q);
