...
%% Loop
clc
size_A = zeros(n_problem, 2);
nb_slack_var = zeros(n_problem, 1);
for i = 1:n_problem
    for j = 1 : length(d1)
        for k = 1 : length(d2)
...
%             o1.solve;
            M(i,:) = size(o1.A);
            nb_slack_var(i) = size(o1.A, 2) - size(lp.A, 2);
            
        end
    end
end

if choice_list_problem == 3
%     save("Name_lp_problem", "list_problem")
%     save("size_mat_A_lp", "M")
    save("nb_slack_variables_lp", "nb_slack_var")
elseif choice_list_problem == 4
%     save("Name_qp_problem", "list_problem")
%     save("size_mat_A_qp", "M")
    save("nb_slack_variables_qp", "nb_slack_var")
end
