classdef nlpmodel_spot < handle
   ...
   methods (Sealed = true)
      ...
      function o = nlpmodel_spot(name, x0, cL, cU, bL, bU)
      ...
      function J = gcon(self, x)
         ...
         dC = spdiags(self.dc,0,self.n,self.n);
         J1 = dR*self.gcon_local(self.dc.*x)*dC;
         J2 = J1';
         J = @(y, mode) ((2-mode) * reshape(J1, (2-mode)*size(J1) + (mode-1)*size(J2)) + (mode-1) * reshape(J2, (2-mode)*size(J1) + (mode-1)*size(J2))  )* y;
%          J = @(y, mode) Jfunction(y, mode, self, dR, dC, x);
